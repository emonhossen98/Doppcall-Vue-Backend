                  ...
                  <div class="row px-3" id="externalFiltersWrapper">
                      ...
                      <ul class="px-0 mt-3" id="offer-extra-filter">
                        ...
                        <li class="position-relative">
                          <a :class="checkfilter.showcolumn == 'Offer Category' ? 'check-active' : ''"
                            @click="clickFilters('category_id', 'select', 'Offer Category')">Offer Category <i
                              class="fa-solid fa-caret-down"></i>
                          </a>
                    ...
                    <div class="col-md-6 ps-4">
                      <div v-if="checkfilter.showcolumn != null && checkfilter.showcolumn != ''">
                        ...
                        <template
                          v-if="checkfilter.showcolumn == 'Offer Category' && checkfilter.types.includes('select')">
                          <div class="form-check mb-2">
                            <input class="form-check-input" @change="clickCheckboxFilters(checkfilter.showcolumn)"
                            ...
                            <template
                              ...
                              <template v-if="fillterDatas?.categoryOffers?.length > 0">
                                <template v-for="(category, index) in fillterDatas?.categoryOffers" :key="index">
                                  <div class="form-check mb-2">
                                    <input @change="clickCheckboxFilters(checkfilter.showcolumn)"
                                      v-model="checkfilter.selectedValues['category_id']" class="form-check-input"
                                      type="checkbox" :value="category.id" :id="'dynamicid' + category.id">
...
export default {
  ...
  async mounted() {
    try {
      ...
      if (role == 'Publisher') {
        ...
        this.dynamicSearchId = this.$route.params.id;
  ...
  methods: {
    ...
    getPublisherOffers(id) {
      ...
      axios
        ...
        .then((res) => {
          ...
          if ($.fn.DataTable.isDataTable("#publisher_offers_datatables")) {
            $('#publisher_offers_datatables').DataTable().destroy();
            $('#publisher_offers_datatables').empty();
          }
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
      axios
        .post(
          this.globalVariables.apiUrl + `publisher/offers/category-search-get-data-all-filter/${this.dynamicSearchId}`,
          this.checkfilter,
          {
            headers: { Authorization: "Bearer " + localStorage.getItem("token") }
          }
  ...
  watch: {
    '$route.params.id': {
      immediate: true,
      handler(newVal) {
        this.dynamicSearchId = newVal;
        //this.externalfilterreset();  // data reset
        this.getPusbliserFillterData(); // data fetch
        this.getFiltarOfExtranalFilter();
      }
    }
  }
