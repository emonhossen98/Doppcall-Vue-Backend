...
export default {
  ...
  methods: {
            ...
            var table = $('#pages_tables').DataTable({
              ...
              columns: [
                ...
                // { data: 'slug' },
                {
                  data: "slug",
                  render: function (data, type, row) {
                    return '<a title="https://doppcall.com/' + row.slug + '" target="_blank" href="https://doppcall.com/' + row.slug + '">https://doppcall.com/' + row.slug + '</a>';
                  },
                },
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "admin/pages-search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
    ...
    attachEventListenersBlulkAction() {
      $('#pages_tables').on('change', '.row-checkbox', (event) => {
      ...
      $('#pages_tables thead').on('change', 'input[type="checkbox"]', (event) => {
        const isChecked = event.target.checked;
        $('#pages_tables tbody .row-checkbox').each((index, checkbox) => {
      ...
      $('#bulk-action-select').off().on('change', (e) => {
        ...
        if (action === 'delete') {
          ...
          if (action === "1") {
            ...
            const alertTitle = "Page Want to Publish";
            this.bulkStatusChange(alertTitle);
          }else{
            this.bulkactionids.status = '0';
            const alertTitle = "Page Want to Pending";
            this.bulkStatusChange(alertTitle);
          }
        }
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/pages/bulk/delete",
                this.bulkactionids,
                {
                  headers: {
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/pages/bulk/status",
                this.bulkactionids,
                {
                  headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
