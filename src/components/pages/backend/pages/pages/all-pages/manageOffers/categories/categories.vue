...
export default {
  ...
  methods: {
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
      axios
        .post(
          this.globalVariables.apiUrl + "admin/offers/categories/search-get-data-dal-filter",
          this.checkfilter,
          {
            headers: { Authorization: "Bearer " + localStorage.getItem("token") }
          }
        ...
        .then((res) => {
          // this.$nextTick(() => {
            ...
            const table = $("#categories-tables").DataTable();
            table.clear();
            table.rows.add(res.data.categoryOffers ?? []);
            table.draw();
          } else {
            var formateDate = this.formatDates;
            var table = $('#categories-tables').DataTable({
    ...
    attachEventListenersOfButton() {
      $("#categories-tables_wrapper").on("click", "button", (event) => {
        const target = $(event.target);
