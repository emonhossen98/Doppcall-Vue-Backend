                            ...
                            <div>
                              ...
                              <template v-if="checkfilter.showcolumn == 'Manager' && checkfilter.types.includes('select')">
                                <template v-if="publisherIndex?.accountManagers?.length > 0">
                                    <template v-for="(manager,index) in publisherIndex?.accountManagers" :key="index">
                                      <div class="form-check mb-2">
...
export default {
  ...
  methods: {
                ...
                { data: "name",
                  render: function (data, type, row) {
                    if (row?.name != null && row?.name != "") {
                       return '<a data-vue-route title="'+row?.name+'" href="/admin-manage-publishers-view/'+row?.id+'">'+row?.name+'</a>'; 
                    }
                    return '----------';
                  },
                },
                { data: "fname",
                  render: function (data, type, row) {
                    if (row?.fname != null && row?.fname != "") {
                       return '<a data-vue-route title="'+row?.fname+'" href="/admin-manage-publishers-view/'+row?.id+'">'+row?.fname+'</a>';
                    }
                    return '----------';
                  },
                },
                { data: "lname",
                  render: function (data, type, row) {
                    if (row?.lname != null && row?.lname != "") {
                       return '<a data-vue-route title="'+row?.lname+'" href="/admin-manage-publishers-view/'+row?.id+'">'+row?.lname+'</a>';
                    }
                    return '----------';
                  },
                },
                ...
                { data: "email",
                  render: function (data, type, row) {
                    if (row?.email != null && row?.email != "") {
                       return '<a data-vue-route title="'+row?.email+'" href="/admin-manage-publishers-view/'+row?.id+'">'+row?.email+'</a>'; return '<span title="'+row?.email+'">'+row?.email+'</span>';
                    }
                    return '----------';
                  },
                },
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "admin/manage/publishers/search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
