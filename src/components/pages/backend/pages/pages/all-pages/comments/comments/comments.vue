                    ...
                    <div class="row px-3" id="externalFiltersWrapper">
                      <div class="col-md-6 border-right">
                        <ul class="px-0 mt-3" id="offer-extra-filter">
                          ...
                          <li class="position-relative">
                            <a :class="checkfilter.showcolumn == 'Comment' ? 'check-active' : ''"
                              @click="clickFilters('comment', 'search', 'Comment')">Comment
                            </a>
                            <template v-if="applyfillters.includes('Comment')">
                              <span @click="removeSearch('Comment', 'comment', 'search')"
                                id="remove-to-search-list">x</span>
                            </template>
                          </li>
                          <li class="position-relative">
                      ...
                      <div class="col-md-6 ps-4">
                        <div v-if="checkfilter.showcolumn != null && checkfilter.showcolumn != ''">
                          ...
                          <template v-if="checkfilter.showcolumn == 'Comment' && checkfilter.types.includes('search')">
                            <div class="form-check mb-2">
                              <input class="form-check-input" @change="clickCheckboxFilters(checkfilter.showcolumn)"
                                v-model="checkfilter.emptyValues['comment']" type="checkbox" value="1"
                                id="isemptyvalue">
                              <label class="form-check-label" for="isemptyvalue">
                                is Empty
                              </label>
                            ...
                            <div class="form-check mb-2">
                              <input class="form-check-input" @change="clickCheckboxFilters(checkfilter.showcolumn)"
                                v-model="checkfilter.notemptyValues['comment']" type="checkbox" value="0"
                                id="isnotemptyvalue">
                              <label class="form-check-label" for="isnotemptyvalue">
                                is not Empty
                              </label>
                          ...
                          <div>
                            ...
                            <div>
                              ...
                              <template v-if="checkfilter.showcolumn == 'Comment' && checkfilter.types.includes('search')">
                                <label for="filtertext">Contains</label>
                                <input type="text" @keyup="clickCheckboxFilters(checkfilter.showcolumn)"
                                  v-model="checkfilter.searchValues['comment']" class="form-control mb-2"
                                  id="filtertext" placeholder="Search here..">
                              </template>
                              <template v-if="checkfilter.showcolumn == 'Status' && checkfilter.types.includes('select')">
                              <div class="form-check mb-2">
...
export default {
  ...
  methods: {
        ...
        var table = $('#comments_tables').DataTable({
          ...
          columns: [
            ...
            },
            {
              ...
              render: function (data, type, row) {
                if (row.email != null) {
                  return row.email.length > 20 
                    ? '<span title="'+row.email+'">'+row.email.slice(0, 20) + '...'+'</span>'  
    ...
    attachEventListenersOfButton(){
      $("#comments_tables_wrapper").on("click", "button", (event) => {
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "admin/comments-search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
            ...
            .then((res) => {
              ...
              if ($.fn.DataTable.isDataTable("#comments_tables")) {
                const table = $("#comments_tables").DataTable();
                table.clear();
                table.rows.add(res.data.comments ?? []);
                table.draw();
              } else {
                var formateDate = this.formatDates;
                var table = $('#comments_tables').DataTable({
    ...
    attachEventListenersBlulkAction() {
      $('#comments_tables').on('change', '.row-checkbox', (event) => {
      ...
      $('#comments_tables thead').on('change', 'input[type="checkbox"]', (event) => {
        const isChecked = event.target.checked;
        $('#comments_tables tbody .row-checkbox').each((index, checkbox) => {
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/comments/bulk/delete",
                this.bulkactionids,
                {
                  headers: {
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/comments/bulk/status",
                this.bulkactionids,
                {
                  headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
