<template>
  ...
  <div class="content-wrapper">
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header py-2">
              <h5 class="card-title mb-0 mt-2">Manage Find Us</h5>
            </div>
            <div class="card-body">
              <table class="align-middle mb-0 table table-hover" id="findus_tables">
                <thead>
                  <tr>
                    <!-- <th></th> -->
                    <th></th>
    ...
    <!-- Modal Create -->
    <div class="modal fade" id="FindInfoCreate" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel1">New Find Us Create</h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-footer">
            ...
            <button type="button" @click="manageFindSave" class="btn btn-primary">
    ...
    <!-- Modal Edit -->
    <div class="modal fade" id="FindInfoEdit" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel1">New Find Us Edit</h5>
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
          ...
          <div class="modal-footer">
            ...
            <button type="button" @click="manageFindUpdate()" class="btn btn-primary">
              <i class="fas fa-check fa-sm me-2"></i>
              Update
            </button>
          </div>
...
export default {
      ...
      if (role == 'Super' || role == 'Admin') {
        ...
        this.$nextTick(() => {
          const dataTableWrapper = document.querySelectorAll(
            "#findus_tables_wrapper .row.mx-2"
          );
  ...
  methods: {
    getFindusData() {
      this.getLoader = true;
      axios
        .get(this.globalVariables.apiUrl + "admin/contacts/find-us", {
          headers: { Authorization: "Bearer " + localStorage.getItem("token") },
        })
        .then((res) => {
          if ($.fn.DataTable.isDataTable("#findus_tables")) {
            $("#findus_tables").DataTable().destroy();
          }
          var formateDate = this.formatDates;
          var table = $("#findus_tables").DataTable({
            ...
            initComplete: () => {
              $('#findus_tables').wrap('<div class="commonDataTablesClass"></div>');
              const table = $("#findus_tables").DataTable();
            ...
            columnDefs: [
              ...
              },
              {
                ...
                render: function (data, type, full, meta) {
                  return (
                    '<div class="findus_action d-flex align-items-center"><button title="Edit" type="button" data-bs-toggle="modal" data-bs-target="#FindInfoEdit" id="edit-btn" class="bg-transparent border-0 text-primary me-2" data-id=' +
                    full.id +
            ...
            language: {
              sLengthMenu: "_MENU_",
              search: "",
              searchPlaceholder: "Search Find Us",
              paginate: {
            ...
            buttons: [
              ...
              {
                text:
                  '<span data-bs-toggle="modal" data-bs-target="#FindInfoCreate"><i class="ti ti-plus me-1 ti-xs"></i>Create</span>',
    ...
    attachEventListenersOfButton(){
      $("#findus_tables_wrapper").on("click", "button", (event) => {
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "admin/contacts/find-us-search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
    ...
    attachEventListenersBlulkAction() {
      $('#findus_tables').on('change', '.row-checkbox', (event) => {
      ...
      $('#findus_tables thead').on('change', 'input[type="checkbox"]', (event) => {
        const isChecked = event.target.checked;
        $('#findus_tables tbody .row-checkbox').each((index, checkbox) => {
      ...
      $('#bulk-action-select').off().on('change', (e) => {
        ...
        if (action === 'delete') {
          ...
          if (action === "1") {
            ...
            const alertTitle = "Find us Want to Publish";
            this.bulkStatusChange(alertTitle);
          }else{
            this.bulkactionids.status = '0';
            const alertTitle = "Find us Want to Pending";
            this.bulkStatusChange(alertTitle);
          }
        }
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/contacts/find-us/bulk/delete",
                this.bulkactionids,
                {
                  headers: {
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/contacts/find-us/bulk/status",
                this.bulkactionids,
                {
                  headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    attachEventListeners() {
      $("#findus_tables").on("click", ".findus_action", (event) => {
        const target = $(event.target);
        const dataId = target.data("id");
        const dataClass = target.attr("id");
        if (dataClass === "edit-btn") {
          this.getLoader = true;
          axios.get(this.globalVariables.apiUrl+`admin/contacts/find-us/edit/${dataId}`,  {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("token"),
          ...
          .then((res) => {
            this.manageFindData.id = res.data.findus.id;
            this.manageFindData.name = res.data.findus.name;
            this.manageFindData.status = res.data.findus.status;
          })
          .catch((error) => {
            console.log(error);
          })
    ...
    deleteFindUs(id) {
      Swal.fire({
        ...
        if (result.value) {
          this.getLoader = true;
          axios
            .get(this.globalVariables.apiUrl + `admin/contacts/find-us/delete/${id}`, {
              headers: {
                Authorization: "Bearer " + localStorage.getItem("token"),
              },
            })
    ...
    manageFindSave(){
      this.getLoader = true;
        ...
        .then((res) => {
          ...
          // Close the modal
          const modal = document.getElementById("FindInfoCreate");
    ...
    manageFindUpdate(){
       this.getLoader = true;
         axios.post(this.globalVariables.apiUrl+"admin/contacts/find-us/update", this.manageFindData, {
           headers: {
             Authorization: "Bearer " + localStorage.getItem("token"),
             "Content-Type": "multipart/form-data",
           },
         ...
         .then((res) => {
          ...
          // Close the modal
          const modal = document.getElementById("FindInfoEdit");
          const bootstrapModal = bootstrap.Modal.getInstance(modal);
          if (bootstrapModal) {
            bootstrapModal.hide();
          }
...
<style>
#findus_tables colgroup:nth-of-type(2) {
  display: none !important;
}
#findus_tables .dt-checkboxes-cell {
