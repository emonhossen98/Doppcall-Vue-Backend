...
export default {
  ...
  methods: {
    ...
    attachEventListenersOfButton(){
      $("#industrys_tables_wrapper").on("click", "button", (event) => {
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "admin/contacts/industry-verticals-search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
    ...
    attachEventListenersBlulkAction() {
      $('#industrys_tables').on('change', '.row-checkbox', (event) => {
      ...
      $('#industrys_tables thead').on('change', 'input[type="checkbox"]', (event) => {
        const isChecked = event.target.checked;
        $('#industrys_tables tbody .row-checkbox').each((index, checkbox) => {
      ...
      $('#bulk-action-select').off().on('change', (e) => {
        ...
        if (action === 'delete') {
          ...
          if (action === "1") {
            ...
            const alertTitle = "Industry Want to Publish";
            this.bulkStatusChange(alertTitle);
          }else{
            this.bulkactionids.status = '0';
            const alertTitle = "Industry Want to Pending";
            this.bulkStatusChange(alertTitle);
          }
        }
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/contacts/industry-verticals/bulk/delete",
                this.bulkactionids,
                {
                  headers: {
      ...
      Swal.fire({
        ...
        if (result.value) {
          (this.getLoader = true),
            axios
              .post(
                this.globalVariables.apiUrl + "admin/contacts/industry-verticals/bulk/status",
                this.bulkactionids,
                {
                  headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
