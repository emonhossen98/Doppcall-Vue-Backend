          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive-sm table-overflow-hidden">
                <table class="align-middle mb-0 table table-hover" id="author_pages_tables">
                  <thead>
                    <tr>
                      <!-- <th></th> -->
                      <th></th>  
...
export default {
  ...
  async mounted() { 
    try {
      ...
      if (role == 'Author') {
        this.getPagesData();
        this.$nextTick(() => {
          const dataTableWrapper = document.querySelectorAll('#author_pages_tables_wrapper .row.mx-2');
  ...
  methods: {
    getPagesData(page = 1, perPage = 10,searchValue = '') {
      ...
      axios
        ...
        .then((res) => {
          ...
          if ($.fn.DataTable.isDataTable("#author_pages_tables")) {
            $('#author_pages_tables').DataTable().destroy();
          }
          var formateDate = this.formatDates;
            var table = $('#author_pages_tables').DataTable({
              data: data,
              columns: [
                // { data: 'id' },
                { data: 'id' },
              ...
              },
              {
                 data: "updated_at",
                  render: function (data, type, row) {
                    return '<div class="d-flex align-items-center"><div class="dropdown mr-1"><button aria-expanded="false" aria-haspopup="true" title="Action" class="border-0 action-btn text-dark"  data-bs-toggle="dropdown" id="dropdownMenuButton" type="button"><i class="fa-solid fa-ellipsis-vertical"></i></button><div class="dropdown-menu tx-13"><button class="dropdown-item view-btn" data-id="'+row.slug+'"><i class="fas fa-eye me-1 text-info" data-id="'+row.slug+'"></i></i>View</button><button class="dropdown-item edit-btn" data-id="'+row.id+'"><i data-id="'+row.id+'" class="far fa-edit fa-sm me-1 text-primary"></i>Edit</button><button class="dropdown-item delete-btn" data-id="'+row.id+'"> <i data-id="'+row.id+'" class="far fa-trash-alt fa-sm me-1 text-danger"></i>Delete</button></div></div></div>';
                  }
              }
                
              ],
              initComplete: () => { 
                $('#author_pages_tables').wrap('<div class="commonDataTablesClass"></div>');
                const table = $("#author_pages_tables").DataTable();
              ...
              const searchInput = $("#author_pages_tables_filter input");
    ...
    attachEventListeners() {
      $("#author_pages_tables").on("click", ".dropdown-item, .action-btn", (event) => {
    ...
    attachEventListenersOfButton(){
      $("#author_pages_tables_wrapper").on("click", "button", (event) => {
        const target = $(event.target);
    ...
    attachEventListenersForMenu() {
      $("#author_pages_tables_wrapper [name='author_pages_tables_length']").on("change", (event) => {
        this.getLoader = true;
        const target = $(event.target);
    ...
    attachEventListenersForSearch() {
      $("#author_pages_tables_wrapper #author_pages_tables_filter input").on("keyup", (event) => {
        const target = $(event.target);
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "author/pages/search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
...
#author_pages_tables colgroup:nth-of-type(2) {
	display: none !important;
}
#author_pages_tables .dt-checkboxes-cell{
	padding: 0.7rem 0.5rem !important;
}
#dropdownMenuButton {
	background: transparent;
}
#author_pages_tables .dropdown-item {
