        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header py-3">
              <h5 class="card-title mb-0 mt-3">Press Title</h5>
            </div>
            <div class="card-body">
              <form>
                <div class="row-mb">
          ...
          <div class="card mt-4">
            <div class="card-header py-2">
              <h5 class="card-title mt-2 mb-0">Press Posts</h5>
            </div>
            <div class="card-body">
...
export default {
  ...
  methods: {
    TitleSave() {
      this.getLoader = true;
      axios
        .post(this.globalVariables.apiUrl + "admin/pressPost/title", this.pressTitles, {
          headers: {
    ...
    getPostData() {
      this.getLoader = true;
      axios
        .get(this.globalVariables.apiUrl + "admin/pressPost/index", {
          headers: { Authorization: "Bearer " + localStorage.getItem("token") },
        })
        .then((res) => {
          this.pressTitles.title = res.data.press_title;
          this.pressTitles.subtitle = res.data.press_subtitle;
          var appUrl = this.globalVariables.appUrl;
          var formatDate = this.formatDate;
          if ($.fn.DataTable.isDataTable("#posts_tables")) {
            $("#posts_tables").DataTable().destroy();
          }
          var table = $("#posts_tables").DataTable({
            data: res.data.press_posts,
            columns: [
              // { data: 'id' },
              { data: "id" },
              { data: "id" },
                ...
                },
              {
                  ...
                  render: function (data, type, row) {
                    if(row.feature_image != null){
                      return '<img class="press_image" src="'+appUrl+row.feature_image+'" alt="image">'
                    }else{
                      return 'No Image';
                    }
                  },
              ...
              },
              {
                ...
                render: function (data, type, full, meta) {
                  return (
                    '<div class="posts_action d-flex align-items-center"><a data-vue-route title="Edit" href="/admin-press-posts-edit/'+full.id+'" class="bg-transparent border-0 text-primary me-2"><i class="far fa-edit fa-sm"></i></a><button type="button" id="delete-btn"  data-id=' +
                    full.id +
            ...
            language: {
              sLengthMenu: "_MENU_",
              search: "",
              searchPlaceholder: "Search Press Post",
              paginate: {
  ...
  getFiltarOfExtranalFilter(perPage = 10,) {
        axios
          .post(
            this.globalVariables.apiUrl + "admin/pressPost/search-get-data-all-filter",
            this.checkfilter,
            {
              headers: { Authorization: "Bearer " + localStorage.getItem("token") }
            }
          ...
          .then((res) => {
            ...
            if ($.fn.DataTable.isDataTable("#posts_tables")) {
              const table = $("#posts_tables").DataTable();
              table.clear();
              table.rows.add(res.data.press_posts ?? []);
              table.draw();
            } else {
              var table = $("#posts_tables").DataTable({
                data: res.data.press_posts,
  ...
  attachEventListenersOfButton() {
    $("#posts_tables_wrapper").on("click", "button", (event) => {
      ...
      const dataClass = target.attr("id");
      if (dataClass === "create") {
        this.$router.push("/admin-press-posts-create");
  ...
  deleteQuestions(id) {
    Swal.fire({
      ...
      if (result.value) {
        this.getLoader = true;
        axios
          .delete(this.globalVariables.apiUrl + `admin/pressPost/destroy/${id}`, {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("token"),
            },
          })
