<template>
            ...
            var table = $('#press_categiories_datatables').DataTable({
              ...
              columns: [
              ...
              },
                {
                  ...
                  render: function (data, type, row) {
                    return (
                      '<div class="type-datatables-action"><a data-vue-route href="/admin-press-categories-edit/'+row.id+'" title="Edit" class="bg-transparent border-0 text-primary me-2"><i class="far fa-edit fa-sm"></i></a><button title="Delete" type="button" id="delete-category"  data-id='+ row.id + ' class="bg-transparent border-0 text-danger"><i  data-id="' + row.id + ' " id="delete-category" class="far fa-trash-alt fa-sm"></i></button></div>'
                    );
                  },
                },
              ],
  ...
  getFiltarOfExtranalFilter(perPage = 10,) {
        axios
          .post(
            this.globalVariables.apiUrl + "admin/pressCategories/search-get-data-all-filter",
            this.checkfilter,
            {
  ...
  attachEventListenersBlulkAction() {
    ...
    $('#press_categiories_datatables thead').on('change', 'input[type="checkbox"]', (event) => {
      const isChecked = event.target.checked;
      $('#press_categiories_datatables tbody .row-checkbox').each((index, checkbox) => {
    ...
    $('#bulk-action-select').off().on('change', (e) => {
      ...
      if (action === 'delete') {
        ...
        if (action === "1") {
          ...
          const alertTitle = "Press Category Want to Publish";
          this.bulkStatusChange(alertTitle);
        }else{
          this.bulkactionids.status = '0';
          const alertTitle = "Press Category Want to Pending";
          this.bulkStatusChange(alertTitle);
        }
      }
    ...
    Swal.fire({
      ...
      if (result.value) {
        (this.getLoader = true),
          axios
            .post(
              this.globalVariables.apiUrl + "admin/pressCategories/bulk/delete",
              this.bulkactionids,
              {
                headers: {
                  Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    Swal.fire({
      ...
      if (result.value) {
        (this.getLoader = true),
          axios
            .post(
              this.globalVariables.apiUrl + "admin/pressCategories/bulk/status",
              this.bulkactionids,
              {
                headers: {
                  Authorization: "Bearer " + localStorage.getItem("token"),
