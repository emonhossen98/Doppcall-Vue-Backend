...
export default {
  ...
  methods: {
      ...
      axios
        ...
        var formateDate = this.formatDates;
            var table = $('#media_tables').DataTable({
              ...
              initComplete: () => { 
                $('#media_tables').wrap('<div class="commonDataTablesClass"></div>');
                const table = $("#media_tables").DataTable();
    ...
    getFiltarOfExtranalFilter(perPage = 10,) {
          axios
            .post(
              this.globalVariables.apiUrl + "admin/settings/media-search-get-data-all-filter",
              this.checkfilter,
              {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
              }
